...
def assert_gwas_extracted_from_data_3(unit_test, gwas):
    expected_snp = pandas.Series(["rs3", "rs6", "rs7"], dtype=numpy.str_)
    numpy.testing.assert_array_equal(gwas[SNP], expected_snp)

    expected_effect = pandas.Series(["G", "G", "T"], dtype=numpy.str_)
    numpy.testing.assert_array_equal(gwas[EFFECT_ALLELE], expected_effect)

    expected_non_effect = pandas.Series(["A", "A", "C"], dtype=numpy.str_)
    ...
    expected_chromosome = pandas.Series(["chr1", "chr1", "chr1"], dtype=numpy.str_)
...
class TestGWAS(unittest.TestCase):
    ...
    def test_gwas_from_source(self):
        ...
        numpy.testing.assert_array_equal(gwas[SNP], pandas.Series(["rs940550", "rs6650104", "rs61770173", ], dtype=numpy.str_))
        numpy.testing.assert_array_equal(gwas[EFFECT_ALLELE], pandas.Series(["C", "T",  "A"], dtype=numpy.str_))
        numpy.testing.assert_array_equal(gwas[NON_EFFECT_ALLELE], pandas.Series(["G", "C", "C"], dtype=numpy.str_))
        numpy.testing.assert_array_equal(gwas[CHROMOSOME], pandas.Series(["chr1", "chr1",  "chr22"], dtype=numpy.str_))
